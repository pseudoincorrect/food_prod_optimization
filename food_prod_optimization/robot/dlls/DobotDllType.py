...
def load():
    if platform.system() == "Windows":
        ...
        # Separation of DLLs from the application code
        dll_name = "DobotDll.dll"
        path = pathlib.Path(__file__)
        dlls_dir_path = path.parent.absolute() 
        dobot_dll_path = dlls_dir_path  /  pathlib.Path(dll_name)
        print("Adding dlls/ folder to PATH")
        os.environ['PATH'] = str(dlls_dir_path) + os.pathsep + os.environ['PATH']
        return CDLL(str(dobot_dll_path),  RTLD_GLOBAL)
